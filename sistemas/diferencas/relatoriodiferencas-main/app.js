...
import { auth, db } from "../firebaseConfig.js"; // ajuste o caminho se estiver em outra pasta
import {
  onAuthStateChanged,
  signOut,
  updatePassword,
...
onAuthStateChanged(auth, async (user) => {
  if (!user) {
    location.href = "../index.html"; // volta pro portal se não estiver logado
    return;
  }
...
async function salvarRelatorioAdmin() {
  ...
  if (!matricula || !data) {
    alert("Preencha matrícula e data.");
    return;
  }
  try {
    const sobra = vd - vf;
